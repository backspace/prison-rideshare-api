defmodule PrisonRideshareWeb.RideController do
  ...
  plug PrisonRideshareWeb.Plugs.Admin when not action in [:index, :update, :calendar]
  ...
  def calendar(conn, _) do
    rides = Repo.all from r in Ride, where: r.enabled and is_nil(r.distance) and is_nil(r.combined_with_ride_id) and is_nil(r.driver_id), preload: [:institution, :driver, :children]

    events = Enum.map(Enum.sort_by(rides, fn(ride) -> ride.start end), fn(ride) ->
      request_count = 1 + length(ride.children)
      passenger_count = Enum.map([ride] ++ ride.children, fn(ride) -> ride.passengers end)
      |> Enum.sum

      summary = "#{ride.institution.name}: #{request_count} request#{if request_count > 1 do "s" end}#{if passenger_count > 1 do ", #{passenger_count} passengers" end}"
