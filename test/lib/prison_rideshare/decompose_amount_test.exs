...
defmodule PrisonRideshare.DecomposeAmountTest do
  ...
  test "it finds an exact match" do
    ...
    assert DecomposeAmount.decompose_amount(~M[100], [other_component, matching_component]) == [matching_component]
  end
  ...
  test "it finds components that sum to the amount" do
    ...
    assert DecomposeAmount.decompose_amount(~M[150], [first_component, second_component]) == [first_component, second_component]
  end
  ...
  test "it finds components that sum to the amount among many" do
    ...
    assert DecomposeAmount.decompose_amount(~M[1010], [one, ten, hundred, thousand]) == [ten, thousand]
  end
